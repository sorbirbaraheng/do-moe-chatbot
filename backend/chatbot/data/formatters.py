...
from ..core.types import SearchResult, ParsedQuery, QueryIntent, QueryLevel
...
class ResponseFormatter:
    ...
    def _generate_ai_insight(self, result: SearchResult, parsed_query: ParsedQuery) -> Generator[str, None, None]:
        ...
        try:
            ...
            prompt = f"""
role: คุณคือ "น้องดีโอ" (DO-MOE) ผู้ช่วยสถิติการศึกษาไทย
style: นุ่ม สุภาพ เป็นกันเอง ใช้ "ครับ" พอดี ๆ ไม่เยิ่นเย้อ
task: สรุปอินไซต์จากข้อมูลแบบกระชับ 2–3 ประโยค
rules:
- ห้ามเกริ่นยาว ห้ามพูดว่า "จากฐานข้อมูล..."
- โฟกัสภาพรวม/จุดเด่น 1 อย่าง
- ไม่ต้องใส่อีโมจิ
...
style: นุ่ม สุภาพ เป็นกันเอง แบบงานบริการ
situation: ไม่พบข้อมูลเฉพาะทาง จึงตอบด้วยความรู้ทั่วไป
task: ตอบสั้น ๆ 3–5 ประโยค
rules:
- หลีกเลี่ยงคำว่า "ไม่พบข้อมูล/ไม่มีข้อมูลในฐานข้อมูล"
- ถ้าข้อมูลเฉพาะมาก ให้ตอบกลาง ๆ แล้วชวนระบุรายละเอียดเพิ่ม 1 คำถามสั้น ๆ
- ไม่ต้องใส่อีโมจิ
- ถ้าไม่ได้ถามเรื่องปี/ช่วงเวลา ห้ามพูดถึงปี
...
"""
    ...
    def _format_ranking(self, result: SearchResult, level: QueryLevel, parsed_query: ParsedQuery) -> Generator[str, None, None]:
        ...
        yield f"### ผลการจัดอันดับ{loc_type}ที่มีโรงเรียน{title}\n\n"
        yield "สรุปผลตามนี้ครับ\n\n"
    ...
    def _format_single(self, result: SearchResult, level: QueryLevel, parsed_query: ParsedQuery) -> Generator[str, None, None]:
        ...
        yield f"### ข้อมูลจำนวนโรงเรียนใน {display}\n\n"
